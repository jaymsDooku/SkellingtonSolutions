...
namespace TextAdventuresCS
{
    class Program
    {
        ...
        private static List<Item> getInventoryItems(List<Item> items)
        {
            List<Item> inventory = items.FindAll(
                    delegate(Item item)
                    {
        ...
        private static bool hasItem(List<Item> items, String name)
        {
        ...
        private static void HitCharacter(string instruction, List<Item> items, List<Character> characters)
        {
            List<Item> inventoryItems = getInventoryItems(items);

            if (!hasItem(items, "truncheon")) 
            {
                Console.WriteLine("You must be holding a truncheon to hit another character.");
                return;
            }

            Character toHit = getCharacter(characters, "guard");

            if (toHit == null)
            {
                Console.WriteLine("That character doesn't exist.");
                return;
            }

            if (toHit.CurrentLocation != characters[0].CurrentLocation)
            {
                Console.WriteLine("That character isn't in the same room.");
                return;
            }

            toHit.isHit = true;
            Console.WriteLine("You hit " + toHit.Name + " round the head and they are now bleeding to death on the ground.");
        }
            ...
            while (!stopGame)
            {
                ...
                switch (Command)
                {
                    ...
                    case "quit":
                        ...
                        stopGame = true;
                        break;
                    case "hit": // hit the guard
                        HitCharacter(instruction, items, characters); // use examine guard to search the fella while he's down
                        break;
                    case "steal": // steal the guard's items
                        StealItem(instruction, items, characters);
                        break;
                    default:
